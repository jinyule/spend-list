...
import com.spendlist.app.domain.usecase.renewal.ReconcileNextRenewalDatesUseCase
...
class ExpirationCheckWorker @AssistedInject constructor(
    ...
    private val reconcileNextRenewalDates: ReconcileNextRenewalDatesUseCase,
    private val markExpiredSubscriptions: MarkExpiredSubscriptionsUseCase
) : CoroutineWorker(appContext, workerParams) {

    override suspend fun doWork(): Result {
        return try {
            // Reconcile stale nextRenewalDate before classifying expirations so a
            // subscription that was only stale-expired (not actually overdue) is
            // not re-flagged as EXPIRED.
            reconcileNextRenewalDates()
